...
import { faCode, faBrain, faCogs, faChartLine, faTrash, faEdit, faPlus } from '@fortawesome/free-solid-svg-icons';
...
const ServiceForm = () => {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => {
      toast({
        title: "Erreur",
        description: error.message || "Une erreur s'est produite lors de la création du service.",
        variant: "destructive",
      });
    }
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      toast({
        title: "Erreur",
        description: error.message || "Une erreur s'est produite lors de la mise à jour du service.",
        variant: "destructive",
      });
    }
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => {
      toast({
        title: "Erreur",
        description: error.message || "Une erreur s'est produite lors de la suppression du service.",
        variant: "destructive",
      });
    }
  });
  ...
  return (
    <div className="space-y-8">
      <Card>
        <CardHeader>
          <CardTitle>{isEditing ? 'Modifier le service' : 'Ajouter un service'}</CardTitle>
          <CardDescription>
            {isEditing ? 'Modifiez les informations du service' : 'Ajoutez un nouveau service à la liste'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
                        ...
                        <SelectContent className="bg-gray-800 border-gray-700 text-white">
                          <SelectItem value="fa-code">
                            <div className="flex items-center">
                              <FontAwesomeIcon icon={faCode} className="mr-2" /> Code
                            </div>
                          </SelectItem>
                          <SelectItem value="fa-brain">
                            <div className="flex items-center">
                              <FontAwesomeIcon icon={faBrain} className="mr-2" /> Cerveau
                            </div>
                          </SelectItem>
                          <SelectItem value="fa-cogs">
                            <div className="flex items-center">
                              <FontAwesomeIcon icon={faCogs} className="mr-2" /> Engrenages
                            </div>
                          </SelectItem>
                          <SelectItem value="fa-chart-line">
                            <div className="flex items-center">
                              <FontAwesomeIcon icon={faChartLine} className="mr-2" /> Graphique
                            </div>
                          </SelectItem>
                        </SelectContent>
                      </Select>
                      ...
                      >
                        ...
                        <SelectContent className="bg-gray-800 border-gray-700 text-white">
                          <SelectItem value="blue">
                            <div className="flex items-center">
                              <div className="w-4 h-4 rounded-full bg-[#0080FF] mr-2"></div> Bleu
                            </div>
                          </SelectItem>
                          <SelectItem value="yellow">
                            <div className="flex items-center">
                              <div className="w-4 h-4 rounded-full bg-[#FFC000] mr-2"></div> Jaune
                            </div>
                          </SelectItem>
                        </SelectContent>
                      </Select>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Liste des services affichés sur le site</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {services.length === 0 ? (
              <p className="text-center text-gray-400 py-4">Aucun service disponible</p>
            ) : (
              services.map((service) => (
                ...
                >
                  <div className="flex items-start">
                    <div className={`p-3 bg-[${service.color === 'blue' ? '#0080FF' : '#FFC000'}]/10 rounded-full mr-4`}>
                      <FontAwesomeIcon 
