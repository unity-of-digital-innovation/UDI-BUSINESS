...
const ProjectForm = () => {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => {
      toast({
        title: "Erreur",
        description: error.message || "Une erreur s'est produite lors de la création du projet.",
        variant: "destructive",
      });
    }
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (error) => {
      toast({
        title: "Erreur",
        description: error.message || "Une erreur s'est produite lors de la mise à jour du projet.",
        variant: "destructive",
      });
    }
  });
  ...
  return (
    <div className="space-y-8">
      <Card>
        <CardHeader>
          <CardTitle>{isEditing ? 'Modifier le projet' : 'Ajouter un projet'}</CardTitle>
          <CardDescription>
            {isEditing ? 'Modifiez les informations du projet' : 'Ajoutez un nouveau projet à la galerie'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Liste des projets affichés sur le site</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {projects.length === 0 ? (
              <p className="text-center text-gray-400 py-4 col-span-full">Aucun projet disponible</p>
            ) : (
